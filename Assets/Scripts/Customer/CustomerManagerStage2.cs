...
public class CustomerManagerStage2 : MonoBehaviour
{
    ...
    {
        if (foodReceive.IsFoodChopped() || foodReceive.IsFoodGriled() || foodReceive.IsFoodAlert())
        {
        ...
        if (customerOrders.Count > 0 && foodReceive)
        {
            foreach (var item in customerOrders)
            {
                ...
                if (item.GetOrderId() == foodReceive.GetFoodItemId() && foodReceive.IsFoodChopped() ||
                    item.GetOrderId() == foodReceive.GetFoodItemId() && foodReceive.IsFoodGriled() ||
